...
class Plotter:
    ...
    def plot_fnonnat(self, path=None):
        ...
        self._place_axis_legend(ax, title="Mapping")
        ax.set_title('Time Evolution of Fraction of Non-Native Contacts ($f_{nonnat}$)')
    ...
    def plot_fnat(self, path=None):
        ...
        self._place_axis_legend(ax, title="Mapping")
        ax.set_title('Time Evolution of Fraction of Native Contacts ($f_{nat}$)')
    ...
    def _load_interface_table(self, path=None):
        ...
        if df is None:
            return None
        expected_labels = ["Interior", "Surface", "Support", "Rim", "Core"]
        label_cols = [col for col in expected_labels if col in df.columns]
        if not label_cols:
            print("[warn] res_interface_stats.csv is missing label columns (Interior, Surface, etc.). Skipping plot.")
            return None
        ...
        if "interface" in df.columns:
            ...
            cols_to_sum = [c for c in label_cols if c in ("Support", "Rim", "Core")]
            df["Interface_score"] = df[cols_to_sum].sum(axis=1)

        name_to_id = {"Interior": 0, "Surface": 1, "Support": 2, "Rim": 3, "Core": 4}
